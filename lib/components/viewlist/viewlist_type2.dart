...
class ViewListType2 extends StatefulWidget {

  final ViewList viewList;

  ViewListType2({required this.viewList});

  @override
  State<ViewListType2> createState() => _ViewListType2State();
}

class _ViewListType2State extends State<ViewListType2> {
  late List<Tag> tagsContent;
  late Tag tagContent;
  ...
  void initState() {
    super.initState();
    tagsContent = (widget.viewList.data!['tags'] as List).map((item) => Tag.fromJson(item)).toList();
      ...
      children: [
        ...
        Container(
          ...
          child: ListView.builder(
            ...
            itemCount: tagsContent.length,
            itemBuilder: (BuildContext context, int index) {
              return Row(
                children: [
                  if (index == 0) SizedBox(width: 5), // 如果索引为0，那么就添加一个宽度为10的 SizedBox
                  Container(
                    width: 150*16/9, // 给 TCardText 设置一个固定的宽度
                    child: AspectRatio(
                      aspectRatio: 16 / 9,
                      child: TCardText(tag: tagsContent[index]),
                    ),
                  ),
                ],
              );
